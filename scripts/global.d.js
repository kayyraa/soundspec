...
globalThis.Hint = document.querySelector(".Hint");
...
globalThis.ShowHint = (Text = "", Duration = 250) => {
    HintDict.push(`${Text}>${Duration}`)
    Hint.style.opacity = "1";
    Hint.innerHTML = Text;
    setTimeout(() => Hint.style.opacity = "0", 250 + Duration);
};

globalThis.HideHint = () => Hint.style.opacity = "0";

let FadeTimeout;

const SetFade = () => {
    clearTimeout(FadeTimeout);
    FadeTimeout = setTimeout(() => Dashboard.setAttribute("blur", ""), 1250);
};

Dashboard.addEventListener("mouseenter", () => {
    clearTimeout(FadeTimeout);
    Dashboard.removeAttribute("blur");
});

Dashboard.addEventListener("mouseleave", SetFade);
